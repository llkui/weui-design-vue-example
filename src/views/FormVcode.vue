  ...
  <Page :padding="false" :showFooter="false" type="white">
    <weui-form desc="验证手机号样式" title="验证码">
      <weui-form-group tips="收不到验证码">
        <weui-input :maxlength="11" label="手机号" placeholder="请输入手机号" right-slot v-model="mobile">
          <weui-button type="text">
            <weui-icon :mini="true" @click.native="info()" type="info-circle"></weui-icon>
          </weui-button>
        </weui-input>
        <weui-input label="验证码" placeholder="输入验证码" right-slot v-model="code">
    ...
    <weui-half-screen-dialog :visible="custom" @onClose="close()" title="标题" type="custom">
      <template #content>
        <br />
        <br />可放自定义内容
        <br />
